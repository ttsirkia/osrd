...
fun run(
    ...
    val diagnosisReporter = DiagnosisReporterImpl()
    val blockInfra = simulator.buildBlocks(diagnosisReporter, rawInfra, loadedSignalInfra)
...
private fun signalUpdates(
    ...
    for ((pathSignal, events) in signalAspectChangeEvents) {
        ...
        if (events.first().time != 0L) {
            ...
            val timeStart = envelope.interpolateTotalTimeMS(trainPositionStart.meters)
            signalUpdates.add(
                SignalUpdate(
                    signalId,
                    setOf(),
