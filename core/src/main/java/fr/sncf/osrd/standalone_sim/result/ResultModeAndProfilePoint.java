...
public class ResultModeAndProfilePoint {
    ...
    public ResultModeAndProfilePoint(double start, String usedMode, String usedProfile, String seenMode,
                                     String seenProfile) {
        this.start = start;
    ...
    public static List<ResultModeAndProfilePoint> from(
        ...
        for (var entry : modesAndProfilesUsed.asMapOfRanges().entrySet()) {
            ...
            if (modeAndProfileSeen == null || modeAndProfileSeen.equals(modeAndProfileUsed))
                res.add(new ResultModeAndProfilePoint(entry.getKey().lowerEndpoint(), modeAndProfileUsed.mode(),
                        modeAndProfileUsed.profile(), null, null));
            else
                res.add(new ResultModeAndProfilePoint(entry.getKey().lowerEndpoint(), modeAndProfileUsed.mode(),
                        modeAndProfileUsed.profile(), modeAndProfileSeen.mode(), modeAndProfileSeen.profile()));
        }
        return res;
    }
}
